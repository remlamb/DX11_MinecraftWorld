...
class MapGenerator {
 private:
  vec3_t grass_color = vec3_t(0.1f, 0.9f, 0.1f);
  vec3_t dirt_color = vec3_t(115.f / 256, 63.f / 256, 23.f / 256);
  vec3_t stone_color = vec3_t(120.f / 256, 120.f / 256, 120.f / 256);
  vec3_t water_color = vec3_t(100.f / 256, 110.f / 256, 220.f / 256);
 ...
 public:
  ...
  uint8_t Get_cell(int x, int y, int z) {
    if (x < 0 || x >= chunk_width || y < 0 || y >= chunk_height || z < 0 ||
        z >= chunk_width) {
      return 0;
    }
    return blocks[z * chunk_width * chunk_height + y * chunk_width + x];
  }

  void Set_cell(int x, int y, int z, uint8_t val) {
    if (x < 0 || x >= chunk_width || y < 0 || y >= chunk_height || z < 0 ||
        z >= chunk_width) {
      return;
    }
    blocks[z * chunk_width * chunk_height + y * chunk_width + x] = val;
  }
    ...
    for (int z = 0; z < chunk_width; z++) {
      for (int y = 0; y < chunk_height; y++) {
        for (int x = 0; x < chunk_width; x++) {
          float secondary_noise = +perlin2d(x, z, 0.11f, 1);
          float perlin = perlin2d(x + 100, z, 0.03f, 3) * chunk_height;
          perlin -= 10;
  ...
  void Generate(GeometryBuilder* builder) {
    for (int z = 0; z < chunk_width; z++) {
      for (int y = 0; y < chunk_height; y++) {
        for (int x = 0; x < chunk_width; x++) {
          uint8_t cell_type = Get_cell(x, y, z);
          if (cell_type != 0) {
            vec3_t side_color = {};
            vec3_t top_color = {};
            switch (cell_type) {
              case 1:
                side_color = dirt_color;
                top_color = grass_color;
                break;
              case 2:
                side_color = stone_color;
                top_color = stone_color;
                break;
              case 3:
                side_color = water_color;
                top_color = water_color;
                break;
            }

            // x-
            if (Get_cell(x - 1, y, z) == 0) {
              builder->PushQuad(vec3_t(x, y, z), vec3_t(x, y, z + 1),
                                vec3_t(x, y + 1, z + 1), vec3_t(x, y + 1, z),
                                side_color);
            }
            // x+
            if (Get_cell(x + 1, y, z) == 0) {
              builder->PushQuad(vec3_t(x + 1, y, z), vec3_t(x + 1, y + 1, z),
                                vec3_t(x + 1, y + 1, z + 1),
                                vec3_t(x + 1, y, z + 1), side_color);
            }
            // z+
            if (Get_cell(x, y, z + 1) == 0) {
              builder->PushQuad(vec3_t(x, y, z + 1), vec3_t(x + 1, y, z + 1),
                                vec3_t(x + 1, y + 1, z + 1),
                                vec3_t(x, y + 1, z + 1), side_color);
            }
            // z-
            if (Get_cell(x, y, z - 1) == 0) {
              builder->PushQuad(vec3_t(x, y, z), vec3_t(x, y + 1, z),
                                vec3_t(x + 1, y + 1, z), vec3_t(x + 1, y, z),
                                side_color);
            }
            // y+
            if (Get_cell(x, y + 1, z) == 0) {
              builder->PushQuad(vec3_t(x, y + 1, z), vec3_t(x, y + 1, z + 1),
                                vec3_t(x + 1, y + 1, z + 1),
                                vec3_t(x + 1, y + 1, z), top_color);
            }
            // y-
            if (Get_cell(x, y - 1, z) == 0) {
              builder->PushQuad(vec3_t(x, y, z), vec3_t(x + 1, y, z),
                                vec3_t(x + 1, y, z + 1), vec3_t(x, y, z + 1),
                                side_color);
            }
          }
        }
