...
inputs::input_state g_InputState;
camera camera;
POINT center;
MapGenerator map;
  ...
  switch (msg) {
    ...
    case WM_KEYDOWN: {
      switch (wparam) {
        ...
        case 'D': {
          g_InputState.right = true;
        } break;
        case 'A': {
          g_InputState.left = true;
        } break;
        case 'W': {
          g_InputState.up = true;
        } break;
        case 'S': {
          g_InputState.down = true;
        } break;
        case VK_SPACE: {
          g_InputState.space = true;
        } break;
        case VK_CONTROL: {
          g_InputState.ctrl = true;
        } break;
      }
    } break;
    ...
    case WM_KEYUP: {
      switch (wparam) {
        case 'D': {
          g_InputState.right = false;
        } break;
        case 'A': {
          g_InputState.left = false;
        } break;
        case 'W': {
          g_InputState.up = false;
        } break;
        case 'S': {
          g_InputState.down = false;
        } break;
        case VK_SPACE: {
          g_InputState.space = false;
        } break;
        case VK_CONTROL: {
          g_InputState.ctrl = false;
        } break;
      }
    } break;

    case WM_MOUSEMOVE: {
      g_InputState.mouse_pos_x = GET_X_LPARAM(lparam);
      g_InputState.mouse_pos_y = GET_Y_LPARAM(lparam);
    } break;

    case WM_LBUTTONDOWN: {
      g_InputState.is_mouse_relative = true;
      ShowCursor(FALSE);
    } break;

    case WM_RBUTTONDOWN: {
      g_InputState.is_mouse_relative = false;
      ShowCursor(TRUE);
    } break;
  }
...
int WINAPI WinMain(HINSTANCE instance, HINSTANCE previnstance, LPSTR cmdline,
  ...
  constexpr int MAX_PARTICULES = 100;
  int particules_count = 0;
  ParticuleSystem particule_system_ = ParticuleSystem(MAX_PARTICULES);
  particule_system_.Spawn(vec3_t(1, 1, 1));
  particule_system_.UpdateGeometry();
...
#include "d3d11_vshader.h"
    ...
    const char hlsl[] =
            ...
            "{                                                          \n"
            "    PS_INPUT output;                                       \n"
            "    output.pos = mul(uTransform, float4(input.pos, 1.0f)); \n"
            "    output.uv = input.uv;                                  \n"
  ...
  for (;;) {
    ...
    if (rtView) {
      ...
      // setup 4x4c rotation matrix in uniform
      {
        ...
        g_InputState.CalculateDeltaMousePos();
        //  Update previous mouse position
        g_InputState.old_mouse_pos_x = g_InputState.is_mouse_relative
                                           ? center.x
                                           : g_InputState.mouse_pos_x;
        g_InputState.old_mouse_pos_y = g_InputState.is_mouse_relative
                                           ? center.y
                                           : g_InputState.mouse_pos_y;

        if (g_InputState.is_mouse_relative) {
        ...
        DirectX::XMMATRIX view = camera.UpdateCamera(
            g_InputState.left, g_InputState.right, g_InputState.up,
            g_InputState.down, g_InputState.space, g_InputState.ctrl,
            g_InputState.delta_mouse_pos_x, g_InputState.delta_mouse_pos_y);

        // Projection matrix
        DirectX::XMMATRIX projection = DirectX::XMMatrixPerspectiveFovRH(
            DirectX::XM_PIDIV4, aspect, 1.0f, 1000.0f);
        ;
